...
from theme import (
    ...
    TEXT, TEXT_LOW, TEXT_MID, WEATHER_COLORS, card_title, eyebrow, kpi_card,
    sidebar_status,
)
...
with st.container(key="wml-card-forecast"):
    st.markdown(card_title("Наш прогноз vs официальный (Gismeteo)"),
                unsafe_allow_html=True)
    fig = go.Figure()
    if obs:
        ...
        if not df_obs.empty:
            ...
            fig.add_trace(go.Scatter(
                ...
                line=dict(color="#7b8798", width=2, shape="spline", smoothing=1.0),
            ))
    ...
    if gismeteo:
        ...
        if not df_g.empty:
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="lines+markers", name="Gismeteo (на завтра)",
    ...
    if temp is not None:
        fig.add_trace(go.Scatter(
            ...
            mode="markers", name=f"Наш прогноз (+{horizon} ч)",
    ...
    fig.update_layout(
        height=380,
        margin=dict(t=10, b=30, l=58, r=20),
...
# --- What-if (title + intro + form + result — ONE card) ---
st.markdown(
    f'<h3 style="font-size: 18px; font-weight: 700; letter-spacing: -.02em;'
    f'margin: 0 0 6px;">Что если…</h3>'
    f'<p style="font-size: 13px; color: {TEXT_LOW}; margin: 0 0 14px;">'
    f'Подставьте свои значения и посмотрите как изменится прогноз модели.</p>',
    unsafe_allow_html=True,
)
...
with st.container(key="wml-card-whatif"):
    ...
    if submitted:
        ...
        try:
            ...
            mc_color = WEATHER_COLORS.get(mc, TEXT_MID)
            delta_html = (
                f' <span style="font-size:12px;color:{TEXT_LOW};">'
                f'({(mt - temp):+.1f}°C к авто)</span>'
                if mt is not None and temp is not None else ""
            )
            st.markdown(
                f"""<div style="margin-top:14px; padding:14px 16px; border-radius:12px;
                background:rgba(110,197,214,0.05); border:1px solid rgba(110,197,214,0.18);
                display:flex; flex-wrap:wrap; gap:24px; align-items:center;">
                <div><span style="font-size:12px;color:{TEXT_LOW};">T° через +{horizon} ч: </span>
                <span style="font-family:'JetBrains Mono',monospace;font-size:20px;font-weight:600;color:{ACCENT_LIGHT};">{mt:.1f}°C</span>{delta_html}</div>
                <div><span style="font-size:12px;color:{TEXT_LOW};">P(дождь): </span>
                <span style="font-family:'JetBrains Mono',monospace;font-size:20px;font-weight:600;color:{RAIN_LIGHT};">{mp*100:.0f}%</span></div>
                <div style="display:flex;align-items:center;gap:8px;">
                <span style="width:11px;height:11px;border-radius:3px;background:{mc_color};"></span>
                <span style="font-weight:600;">{mc}</span></div>
                </div>""",
                unsafe_allow_html=True,
            )
...
with col_cv:
    with st.container(key="wml-card-cv"):
        st.markdown(card_title("Метрики кросс-валидации"), unsafe_allow_html=True)
        if metrics:
            h_str = str(horizon)
            ...
            st.markdown(
                f'<div style="font-size:12px; color:{TEXT_LOW}; '
                f'font-family:\'JetBrains Mono\',monospace; margin: -4px 0 12px;">'
                f'Версия модели · <span style="color:{SUCCESS};">{version}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
            ...
            items = [
                ("T° CV-MAE",
                 f"{t_m.get('cv_mae_mean', float('nan')):.2f} °C" if t_m else "—",
                 ACCENT_LIGHT, f"naive: {t_m.get('naive_mae', 0):.2f}" if t_m else ""),
                ("Дождь CV-Acc",
                ...
                ("Горизонт", f"+{horizon} ч", TEXT, ""),
            ]
            # 2×2 grid INSIDE the same card — no inner bordered boxes,
            # just labels + values laid out as a grid (matches mockup).
            grid_html = "".join(
                f'<div>'
                f'<div style="font-size:11px; color:{TEXT_LOW}; margin-bottom:6px;">{label}</div>'
                f'<div style="font-family:\'JetBrains Mono\',monospace; font-size:24px;'
                f' font-weight:600; color:{clr}; line-height:1;">{val}</div>'
                f'<div style="font-size:11px; color:{MUTED}; margin-top:6px;'
                f' font-family:\'JetBrains Mono\',monospace;">{sub}</div>'
                f'</div>'
                for label, val, clr, sub in items
            )
            st.markdown(
                f'<div style="display:grid; grid-template-columns:1fr 1fr;'
                f' gap:18px 24px;">{grid_html}</div>',
                unsafe_allow_html=True,
            )
        else:
...
with col_fi:
    with st.container(key="wml-card-fi"):
        st.markdown(card_title("Важность признаков"), unsafe_allow_html=True)
        ...
        if fi:
            ...
            bars_html = "".join(
                ...
                f' font-size:12.5px; margin-bottom:5px;">'
                f'<span style="color:#cbd5e1; font-family:\'JetBrains Mono\',monospace;">'
                f'{row["feature"]}</span>'
                f'<span style="color:#7b8798; font-family:\'JetBrains Mono\',monospace;">'
                f'{row["importance"]:.1f}</span></div>'
                f'<div style="height:7px; border-radius:99px;'
                f' background:rgba(148,163,184,0.1); overflow:hidden;">'
                f'<div style="height:100%; width:{(row["importance"]/max_imp*100):.1f}%;'
                f' border-radius:99px;'
                f' background:linear-gradient(90deg,{PRIMARY_DARK},{PRIMARY});">'
                f'</div></div></div>'
...
# --- MAE per horizon: model vs naive baseline (all 8 horizons at once) ---
temp_horizons = (metrics.get("temperature", {}).get("horizons") or {}) if metrics else {}
if len(temp_horizons) > 1:
    st.markdown("<div style='height: 22px;'></div>", unsafe_allow_html=True)
    with st.container(key="wml-card-mae"):
        st.markdown(card_title(
            "Качество модели по горизонтам (CV-MAE, °C)",
            f'<span style="font-size:12px;color:{TEXT_LOW};">ниже = лучше</span>',
        ), unsafe_allow_html=True)
        h_rows = [
            {"Горизонт, ч": int(h_key),
             "CatBoost": met.get("cv_mae_mean"),
             "Naive (T(t+H)=T(t))": met.get("naive_mae")}
            for h_key, met in temp_horizons.items()
        ]
        df_h = pd.DataFrame(h_rows).sort_values("Горизонт, ч")
        fig_h = go.Figure()
        fig_h.add_trace(go.Scatter(
            x=df_h["Горизонт, ч"], y=df_h["Naive (T(t+H)=T(t))"],
            mode="lines+markers", name="Naive baseline",
            line=dict(color=MUTED, dash="dash", width=2, shape="spline", smoothing=1.0),
            marker=dict(size=8, color=MUTED),
        ))
        fig_h.add_trace(go.Scatter(
            x=df_h["Горизонт, ч"], y=df_h["CatBoost"],
            mode="lines+markers", name="CatBoost (наша модель)",
            line=dict(color=PRIMARY, width=2.6, shape="spline", smoothing=1.0),
            marker=dict(size=9, color=PRIMARY,
                        line=dict(width=2, color="#0e131d")),
        ))
        fig_h.add_vline(
            x=horizon, line_dash="dot", line_color=CORAL,
            annotation_text=f"+{horizon} ч",
